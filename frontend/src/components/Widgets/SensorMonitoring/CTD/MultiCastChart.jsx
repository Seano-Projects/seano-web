...
const METRIC_DEFS = [
  { key: "temperature",    tKey: "temperature",   unit: "°C" },
  { key: "salinity",       tKey: "salinity",      unit: "PSU" },
  { key: "density",        tKey: "density",       unit: "kg/m³" },
  { key: "conductivity",   tKey: "conductivity",  unit: "mS/cm" },
  { key: "sound_velocity", tKey: "soundVelocity", unit: "m/s" },
];
...
const WINDOW_MS = [
  { tKey: "min1",  ms: 60_000 },
  { tKey: "min5",  ms: 300_000 },
  { tKey: "min10", ms: 600_000 },
  { tKey: "min30", ms: 1_800_000 },
];
...
const MultiCastChart = ({ ctdData }) => {
  ...
  const windowOptions = useMemo(
    () => WINDOW_MS.map((w) => ({ ...w, label: t(`pages.ctd.multicast.${w.tKey}`) })),
    [t],
  );

  const [metric, setMetric] = useState("temperature");
  const [windowMs, setWindowMs] = useState(WINDOW_MS[1].ms);

  const metricMeta = metrics.find((m) => m.key === metric) || metrics[0];
  const activeWindow = windowOptions.find((w) => w.ms === windowMs) || windowOptions[1];
  ...
  const { chartData, casts } = useMemo(() => {
    ...
    const rows = Array.from({ length: numBins }, (_, i) => {
      ...
      const row = { depth: depthMin + DEPTH_BIN / 2 };
    ...
    const filteredRows = rows.filter((r) => casts.some(({ key }) => r[key] != null));
    ...
    return (
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-3 shadow-lg min-w-52">
        <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 mb-2">
          {t("pages.data.charts.depth")}: {Number(d.depth).toFixed(1)} m
        </p>
        {payload.map((p) => {
    ...
    <div className="bg-white dark:bg-black border border-gray-200 dark:border-gray-700 rounded-xl p-6">
      <div className="flex flex-wrap items-start justify-between gap-3 mb-4">
        <div>
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
            {t("pages.ctd.charts.allDataSubtitle")} · {casts.length}
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-x-4 gap-y-1 mb-3">
          {casts.map(({ key, label, color }) => (
            <div key={key} className="flex items-center gap-1.5">
              <span className="inline-block w-4 rounded" style={{ backgroundColor: color, height: 2 }} />
      ...
      {casts.length > 0 && (
        <p className="text-xs text-gray-400 dark:text-gray-500 mt-3 text-right">
          {casts.length} {t("pages.ctd.charts.allDataTitle")} · {activeWindow.label}
        </p>
      )}
    </div>
  );
