...
export const Card = ({ children, className = "", style }) => (
  <div
    className={`bg-white dark:bg-black border border-gray-200 dark:border-gray-700 rounded-2xl ${className}`}
    style={style}
  >
    {children}
  </div>
...
export const ForecastRow = ({
  ...
  return (
    <div className="flex items-center gap-3 py-1.5">
      <span className="w-8 text-xs text-gray-500 dark:text-gray-400 shrink-0">
        {dayLabel}
      </span>
      <img src={OWM_ICON(iconCode, "")} alt="" className="w-6 h-6 shrink-0" />
      <span className="w-8 text-xs text-right text-gray-400 dark:text-gray-500 shrink-0">
        {min}°
      </span>
      ...
      <span className="w-8 text-xs text-gray-800 dark:text-gray-200 font-semibold shrink-0">
        {max}°
      </span>
    </div>
  );
...
export const CompareRow = ({ label, min, max, allMin, allMax, bold }) => {
  ...
  return (
    <div className="flex items-center gap-3 py-1.5">
      <span
        className={`w-20 text-xs shrink-0 ${bold ? "font-semibold text-gray-800 dark:text-gray-100" : "text-gray-500 dark:text-gray-400"}`}
      >
        {label}
      </span>
      ...
      <span className="w-6 text-xs font-semibold text-gray-800 dark:text-gray-200 shrink-0">
        {max}°
      </span>
    </div>
  );
