...
const SensorTypeTableSkeleton = ({ rows = 5, columns = [] }) => {
  return (
    <>
      {Array.from({ length: rows }).map((_, idx) => (
        ...
        >
          {columns.map((column, colIndex) => {
            ...
            // Sensor Type column (with icon, name and code)
            if (colIndex === 1) {
              return (
                <td
                  key={colIndex}
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="h-4 bg-gray-300 dark:bg-gray-800 rounded w-36 mb-2"></div>
...
export default SensorTypeTableSkeleton;
