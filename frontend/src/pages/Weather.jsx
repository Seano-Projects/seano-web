...
const Weather = () => {
  ...
  const hasUsvCoords =
    usvStatus === "online" && vehicleLog?.latitude && vehicleLog?.longitude;

  const activeCoords = useMemo(() => {
    if (hasUsvCoords)
      return { lat: vehicleLog.latitude, lon: vehicleLog.longitude };
  ...
  useEffect(() => {
    if (weather) setLastRefreshTime(new Date());
  }, [weather]);
  useEffect(() => {
  ...
  const dailyForecast = useMemo(
    () => buildDailyForecast(forecast?.list || []),
    [forecast],
  );
  const todayHourly = useMemo(
    () => buildTodayHourly(forecast?.list || []),
    [forecast],
  );

  const forecastAllMin = dailyForecast.length
    ? Math.min(...dailyForecast.map((d) => d.min))
    : 0;
  const forecastAllMax = dailyForecast.length
    ? Math.max(...dailyForecast.map((d) => d.max))
    : 30;

  const next24hRain = (forecast?.list || [])
    .slice(0, 8)
    .reduce((s, i) => s + (i.rain?.["3h"] || 0), 0);
  const next24hPop =
    (forecast?.list || []).slice(0, 8).reduce((s, i) => s + (i.pop || 0), 0) /
    Math.max((forecast?.list || []).slice(0, 8).length, 1);
  const nextWeekRain = (forecast?.list || []).reduce(
    (s, i) => s + (i.rain?.["3h"] || 0),
    0,
  );
      ...
      <div className="max-w-5xl mx-auto space-y-5">
        ...
        {weatherError && weatherError !== "no_api_key" && (
          <div className="flex items-center gap-3 p-3 rounded-xl bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-300 text-sm">
            <FaTimesCircle size={14} className="shrink-0" />{" "}
            {t("weather.fetchError")}: {weatherError}
          </div>
        )}
        {geoError && !hasUsvCoords && geoError !== "unsupported" && (
          <div className="flex items-center gap-3 p-3 rounded-xl bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800 text-orange-700 dark:text-orange-300 text-sm">
            ...
            {t("weather.locationError")}: {geoError}
          </div>
        )}
          ...
          <div className="animate-pulse space-y-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {[...Array(2)].map((_, i) => (
                <div
                  key={i}
                  className="h-48 rounded-2xl bg-gray-200 dark:bg-gray-800"
                />
              ))}
            </div>
          </div>
          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <ForecastCard
                ...
                forecastAllMin={forecastAllMin}
                forecastAllMax={forecastAllMax}
              />
              <PrecipCard
                forecast={forecast}
